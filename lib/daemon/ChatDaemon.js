...
class ChatChannel {
    ...
    broadcast(msg, sender, isEmote) {
        ...
        this.listeners.forEach(wp => {
            let p = unwrap(wp), dn = displayName;
            if (!p) return;
    ...
    cmd(args, cmdline, isEmote) {
        ...
        if (args.length === 0) {
            ...
            write(`You are ${(listening ? 'now' : 'no longer')} listening to ${this.display}`);
            return true;
        }
        switch (args[0].toLowerCase()) {
            case "/last": case "-last":
                ...
                if (this.history.length === 0)
                    write('There is no chat history available for ' + this.display);
                return true;
            ...
            case "/who": case "-who":
                ...
                if (list.length === 0)
                    write('No one is listening to ' + this.display);
                else
                    write(`People listening to ${this.display}: ${list.join(', ')}`);
                return true;

            default:
                if (perms === CH_RECEIVE) {
                    write('You cannot send messages on ' + this.display);
                    return true;
                }
...
class ChatDaemon extends MUDObject {
    ...
    addChannel(name, display, userTest) {
        let channels = this.getPrivate('channels', {}),
    ...
    get channels() {
        var channels = this.getPrivate('channels', {});
    ...
    /**
        ...
        */
    getChannel(name) {
        var ch = this.getPrivate('channels', {});
...
module.exports = { ChatChannel, ChatDaemon: new ChatDaemon() };
