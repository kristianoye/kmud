...
class ChatChannel {
        ...
        this.listeners.forEach(wp => {
            ...
            if (p.isListening(this.name)) {
                if (p.hasBrowser) {
                    ...
                    var line = isEmote ?
                        '{0} {1} {2}'.fs(this.display, dn, cmdline.input) :
                        '{0} {1} {2}'.fs(dn, this.display, cmdline.input);
    ...
    cmd(args, cmdline, isEmote) {
        ...
        if (args.length === 0) {
            ...
            tp.writeLine('You are {0} listening to {1}'.fs(listening ? 'now' : 'no longer', this.display));
            return true;
        }
        switch (args[0].toLowerCase()) {
            ...
            case "/who": case "-who":
                ...
                if (list.length === 0)
                    ...
                    tp.writeLine('People listening to {0}: {1}'.fs(this.display, list.join(', ')));
                return true;
...
class ChatDaemon extends MUDObject {
    ...
    constructor(ctx) {
        ...
        this.addChannel('admin', '%^MAGENTA%^[[%^BOLD%^admin%^RESET%^%^MAGENTA%^]]%^RESET%^%^RESET%^', function (p) {
            return efuns.adminp(p) ? CH_SEND | CH_RECEIVE : 0
        });
        this.addChannel('announce', '** ANNOUNCEMENT **', function (p) {
            return efuns.archp(p) ? CH_SEND | CH_RECEIVE : CH_RECEIVE;
        });
        this.addChannel('arch', '%^MAGENTA%^[arch]%^RESET%^', function (p) {
            return efuns.archp(p) ? CH_SEND | CH_RECEIVE : 0;
        });
        this.addChannel('newbie', '[%^GREEN%^newbie%^RESET%^]', function (p) {
            if (efuns.wizardp(p)) return CH_SEND | CH_RECEIVE;
            else if (p.getLevel() < 10) return CH_SEND | CH_RECEIVE;
            else return 0;
        });
        this.addChannel('wiz', '%^BLUE%^[%^RESET%^%^RED%^wiz%^RESET%^%^BLUE%^]%^RESET%^', function (p) {
            return efuns.wizardp(p) ? CH_SEND | CH_RECEIVE : 0;
        });
    }
