...
class EmoteDaemon extends MUDObject {
     ...
     */
    async createAsync() {
        await efuns.restoreObjectAsync(__dirname + '/save/EmoteDaemon.json');
    ...
    doVerbRule(verb, rule, args, parse) {
        ...
        let errors = [], 
            ...
            secondPerson = [efuns.thisPlayer().displayName, efuns.pluralize(verb)],
            thirdPerson = [efuns.thisPlayer().displayName, efuns.pluralize(verb)],
            targets = [];
...
module.exports = await createAsync(EmoteDaemon);
var foo = 1 + 1;
