...
function connectRouter(id, router) {
    let socket = new net.Socket(), buffer = Buffer.alloc(1024);
            ...
            if (line.length === dataLen) {
                ...
                if (Array.isArray(packet) && packet.length > 3) {
                    ...
                    if (fn && typeof this[fn] === 'function') {
                        this[fn].call(this, router, packet);
                    }
                    else {
...
class I3Daemon extends MUDObject {
    ...
    initialize() {
        if (efuns.featureEnabled('intermud3')) {
            ...
            if (routerCount === 0) {
                this.addRouter({ id: 'klf', address: '127.0.0.1' });
            }
        }
    }
...
module.exports = { I3Daemon: new I3Daemon(), I3RouterEntry, I3MUDEntry };
