...
export default final singleton class GroupsCommand extends Command {
    ...
    private async addGroup(id, flags) {
        ...
        do {
            ...
            if (name.length === 0)
                break;
            else if (group.members.indexOf(norm) > -1)
                errorLine(`${name} is already a member of the group`);
            else if (!await efuns.living.userExists(name))
                errorLine(`${name} does not appear to be a valid username`);
            else {
                group.members.push(norm);
                writeLine(`Adding ${name} to group ${id}`);
            }
        }
        while (true);

        writeLine(`Adding group ${id}`);
        return await efuns.security.createSecurityGroup(group);
    }
