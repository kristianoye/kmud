...
class LsCommand extends Command {
    async cmd(text, cmdline) {
        let player = thisPlayer(),
            opts =  await createAsync(ListOptions, player),
        ...
        for (let i = 0; i < targetList.length; i++) {
            try {
                ...
                let stat = await efuns.fs.getObjectAsync(resolved);
    ...
    async displayColumnListing(opts) {
        ...
        if (opts.directories.length > 0) {
            for (let i = 0; i < opts.directories.length; i++) {
                ...
                let subdir = await createAsync(ListOptions, opts.player, opts);
