...
class LsCommand extends Command {
        ...
        for (let i = 0; i < dirs.length; i++) {
            ...
            switch (DisplayFormats[opts.format]) {
                ...
                case DisplayFormats.long:
                    result.push(await this.displayLongListing(opt, dirs.length > 0));
                    break;
     ...
     */
    displayLongListing(dir, displayOptions) {
        try {
            let buffer = '', total = 0;

            if (dir.files.length > 0) {
                dir.files.forEach(fo => {
                    buffer += efuns.sprintf('%5s %10s %10s %10s %-18s',
                        '',
                        'owner',
                        'group',
                        (fo.size === -2) ? '[DIR]' : efuns.getMemSizeString(fo.size),
                        fo.mtime.toISOString().slice(0, 16))
                        + this.displayColumnName(fo, displayOptions) + '\n';
    ...
    displayColumnName(fo, flags) {
        ...
        if (flags & LS_OPT_COLOR) {
            ...
            else if (efuns.findObject(fo.path)) {
                result = "%^GREEN%^%^BOLD%^" + result + "%^RESET%^";
            }
            else if (fo.name.endsWith('.js') || fo.name.endsWith('.jsx')) {
