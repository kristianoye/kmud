...
class FileIndex extends MUDObject {
    ...
    findFiles(opts) {
        ...
        Object.keys(_files).filter(d => d.startsWith(opts.dir)).forEach(d => {
            ...
            if (opts.name) {
                ...
                var files = _files[d].map(fi => d + fi[0]).filter(fn => fn.match(opts.regex));
                if (files.length > 0) result.push(...files);
            }
            else {
                var files = _files[d].map(fi => d + fi[0]);
                if (files.length > 0) result.push(...files);
    ...
    indexFiles(path) {
        efuns.getDir(path, FTYPE_DIRECTORY, (files, err) => {
            if (!err) {
                _files[path] = files;
