 ...
 */
const
    { FTYPE } = await requireAsync('Flags');
...
class FileIndex extends MUDObject {
    ...
    async indexFiles(path) {
        //let err, files = await efuns.readDirectoryAsync(path, FTYPE.Directory).catch(e => err = e);
        //if (!err) {
        //    _files[path] = files;
...
module.exports = await createAsync(FileIndex);
