...
const
    ...
    async = await requireAsync('async');
...
class CommandResolver extends MUDObject {
    ...
    async hashCommandDirectory(...dirs) {
        for (let j = 0, jm = dirs.length; j < jm; j++) {
            ...
            let files = await efuns.fs.readDirectoryAsync(cmdDir);
            ...
            if (Array.isArray(files)) {
                ...
                for (let i = 0, m = files.length; i < m; i++) {
                    ...
                    if (m && m[1]) {
                        try {
                            let cmd = await efuns.objects.loadObjectAsync(`${cmdDir}/${m[1]}`);

                            if (cmd) {
                                let verb = cmd().verbs || m[1].toLowerCase();

                                if (Array.isArray(verb))
                                    verb.forEach(v => dir.addCommand(v, file, cmd));
     ...
     */
    async resolveAsync(cmdName, searchPath) {
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                if (searchPath.indexOf(dir.directory) > -1) {
                    ...
                    if (entry) {
                        ...
                        return entry.bin = await efuns.loadObjectAsync(path);
                    }
                }
            }
        }
    ...
    async updateCommand(cmd) {
        await unwrap(cmd, async o => {
            ...
            await HelpDaemon().addCommand(o, file.toLowerCase());
        });
    }
}
