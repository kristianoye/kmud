...
export default singleton class GameMaster extends MUDObject {
    ...
    isAdmin(target) {
        if (typeof target === 'string')
            ...
            return efuns.security.isGroupMember(`creators\\${target.instance.name}`, "$ADMIN");
        else
            return false;
    }
    ...
    isArch(target) {
        ...
        if (typeof target === 'string')
            ...
            return efuns.security.isGroupMember(`creators\\${target.instance.name}`, "$ASSIST");
        else
            return false;
    }
    ...
    protected aclGetCredential(filename) {
        ...
        switch (pathParts[0]) {
            ...
            case 'world':
                if (pathParts.length > 2) {
                    return { UserId: 'world\\' + pathParts[1].toLowerCase() };
                }
                return { UserId: 'world' };
            case 'wwwroot':
