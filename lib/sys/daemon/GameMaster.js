...
class GameMaster extends MUDObject {
    constructor() {
        ...
        let groupsIn = efuns.readJsonFile(_groupsFile);
    ...
    private compileVirtualObject(path, args) {
        if (path.startsWith('/sys/data/players/')) {
            let player = new Base.Player(path, ...args);
            return player;
        }
        if (path.startsWith('/sys/data/creators/')) {
            let player = new Base.Creator(path, ...args);
            return player;
        }
        return false;
    }
    ...
    connect(port, type, auth = false) {
        try {
            if (auth !== false) {
                ...
                PlayerDaemon().loadPlayer(auth.username, false, player => {
                    if (player) console.log(`Loaded ${player.keyId}`);
                });
            }
            else {
                if (type === 'text')
