...
class GameMaster extends MUDObject {
    ...
    private async startup() {
        let test = await efuns.fs.readJsonFileAsync('/sys/etc/acls.json');
        let createPerms = (expr) => {
            let result = 0;
            if (expr === 'FULL') expr = "rwcdLCDPO";
            else if (expr === 'READ') expr = 'rlL';
            for (let i = 0; i < expr.length; i++) {
                switch (expr[i]) {
                    case 'r': result |= 1 << 0; break; // Read file
                    case 'w': result |= 1 << 1; break; // Write file
                    case 'c': result |= 1 << 2; break; // Create file
                    case 'l': result |= 1 << 3; break; // Load object
                    case 'L': result |= 1 << 4; break; // List dir
                    case 'C': result |= 1 << 5; break; // Create dir
                    case 'D': result |= 1 << 6; break; // Delete dir
                    case 'P': result |= 1 << 7; break; // Change perms
                    case 'O': result |= 1 << 8; break; // Change owner
                }
            }
            return result;
        };
        Object.keys(test).forEach(directory => {
            let permissions = test[directory];
            let list = permissions.split(/\s+/);
        });
    }
