...
export default singleton class GameMaster extends MUDObject {
    ...
    private async compileVirtualObject(path, args) {
        if (path.startsWith('/sys/data/players/')) {
            let player = await createAsync(LIB_PLAYER, path);
            return player;
        }
        if (path.startsWith('/sys/data/creators/')) {
            let creator = await createAsync(LIB_CREATOR, path);
            return creator;
        }
        return false;
    }
    ...
    private async connect(port, type, auth = false) {
        try {
            if (auth !== false) {
                ...
                let test = efuns.english.consolidate(2, 'sword');
                if (type === 'text')
    ...
    isAdmin(target) {
        if (typeof target === 'string')
            ...
            return unwrap(target, u => {
                return efuns.security.isGroupMember(`creators\\${u.name}`, "$ADMIN");
            });
        else
            return false;
    }
    ...
    isArch(target) {
        ...
        if (typeof target === 'string')
            ...
            return unwrap(target, u => {
                return efuns.security.isGroupMember(`creators\\${u.name}`, "$ASSIST");
            });
        else
            return false;
    }
