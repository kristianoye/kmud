...
class PermissionGroup {
     ...
     * @param {PermissionGroup} data Group data
     */
    ...
    /** A description of what the group is for */
    get description() {
        return get('No Description');
    }

    protected set description(desc) {
     ...
     * @param {MUDObject|string} user The user to test for
     * @returns {boolean} Returns true if the user is in the group
     */
    isMember(user) {
        let name = unwrap(user, u => u.name) || user;
        if (typeof name === 'string') {
            name = efuns.normalizeName(name);
            return Array.isArray(this.members) && this.members.indexOf(name) > -1;
        }
        return false;
    }
    ...
    package set members(memberList) {
...
class GameMaster extends MUDObject {
    /** Method called to bootstrap ACL security if used */
    private async bootstrapAclSecurity() {
        return {
            permissionsFile: '/sys/etc/acl-access.yaml',
            groupsFile: '/sys/etc/acl-groups.yaml'
        };
    }
    ...
    private async create() {
        try {
            ...
            let foo = await efuns.fs.getObjectAsync(_accessFile);
            let test = await foo.readJsonAsync();
            ...
            let groups = await efuns.fs.readYamlAsync('/sys/etc/acl-groups.yaml');
            for (let i = 0, names = Object.keys(groups); i < names.length; i++) {
                let name = names[i], groupData = groups[name];
                this.groups[name] = await createAsync(PermissionGroup, groupData);
            }
        }
        catch (ex) {
            console.log(ex);
    ...
    private async compileVirtualObject(path, args) {
        if (path.startsWith('/sys/data/players/')) {
            let player = await createAsync(Base.Player, path);
            await player.loadPlayer(path);
            return player;
        }
        if (path.startsWith('/sys/data/creators/')) {
            let creator = await createAsync(Base.Creator, path);
    ...
    getGroups(user) {
        ...
        return groupNames
            .map(gn => groups[gn].isMember(user) && gn)
            .filter(gn => typeof gn === 'string');
    }
    ...
    getHomePath(user) {
        return unwrap(user, u => {
            if (efuns.living.isWizard(u))
                return `/realms/${u.name}`;
            return undefined;
        });
    }

    /**
     * Get a list of groups for the specified target
     * @param {MUDObject} target
     */
    private async getPermissionGroups(target) {
        let fn = target.filename,
            fileParts = fn.split('/').slice(1),
            groups = [];

        if (fn.startsWith('/sys/data/creators/') || fn.startsWith('/sys/data/players/')) {
            let userName = efuns.normalizeName(fileParts.pop().toLowerCase());
            groups.push(userName);
            groups.push(...this.getGroups(userName));
        }
            ...
            switch (fileParts[0]) {
                ...
                case 'realms':
                    if (fileParts.length > 2)
                        groups.push(fileParts[1].toLowerCase());
                    else
                        groups.push('MUDLIB');
                ...
                case 'world':
                    if (fileParts.length > 2)
                        groups.push(`^${(fileParts[1].toUpperCase())}`);
                    else
                        groups.push('MUDLIB');
                    break;
    ...
    /** @type {Object.<string,PermissionGroup>} */
    get groups() {
        return get({});
    }
            ...
            if (ptr && dir.length > 0) {
                ...
                if (ptr && Array.isArray(ptr['__g'])) {
                    var np = ptr['__g'].map(function (s, n) {
                        return s.fs(tokens); // String.prototype.fs.apply(s, tokens) + '';
                    });
                    result.push(...np);
                }
            }
    ...
    protected async aclCreate(directoryName) {
        return {
            directoryName,
            inherits: true,
        };
    ...
    protected aclGetCredential(filename) {
        let userTest = /\/sys\/data\/(?:players|creators)\/[a-z]{1}\/(?<username>[a-z0-9]+)/,
            userMatch = userTest.exec(filename);

        if (userMatch != null) {
            return {
                IsUser: true,
                UserId: userMatch.groups['username']
            };
        }
        let pathParts = filename.split('/').filter(s => s.length > 0);
        switch (pathParts[0]) {
            case 'base':
                return { UserId: '$mudlib' };
            case 'cmds':
            ...
            case 'realms':
                if (pathParts.length > 2) {
                    return { UserId: pathParts[1].toLowerCase() };
                }
                return { UserId: '$mudlib' };
            case 'sys':
            ...
            case 'world':
                if (pathParts.length > 2) {
                    return { UserId: pathParts[1].toUpperCase() };
                }
                return { UserId: '$mudlib' };
            case 'wwwroot':
                return { UserId: '$wwwroot' };
            default:
