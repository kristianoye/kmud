...
class GameMaster extends MUDObject {
    ...
    inGroup(target, ...group) {
        ...
        if (o || typeof target === 'string') {
            ...
            let gc = g.filter(gn => {
                ...
                return gn in _groups && _groups[gn][name] === true
            });
            return gc.length > 0;
        }
        return false;
    ...
    private async startup() {
        let acls = await efuns.fs.readJsonFileAsync('/sys/etc/acls.json');
        _groups = await efuns.fs.readJsonFileAsync('/sys/etc/groups.json');
        _aclDefs = await efuns.acl.parseAclTree(acls);
        let test = await _aclDefs.effectivePermissions('/sys/etc/acls.json', 'kriton');
        console.log(`Kriton's permissions: ${test}`);
    }
