...
class PlayerDaemon extends MUDObject {
    create() {
        ...
        let test = efuns.driver;
    }

    createAdmin(evt) {
        if (efuns.gameState() < 3) {
            let fn = this.creatorFilename(evt.eventUsername);
            let props = { props: { name: evt.eventUsername, password: evt.eventPassword } };
                ...
                if (!err) {
                    try {
                        ...
                        if (!player) {
                            ...
                            ChatDaemon().broadcast('announce', '{0} has entered the game.'.fs(player().displayName));
                        } else {
                            /* player loaded but linkdead */
                            ChatDaemon().broadcast('announce', '{0} has re-connected.'.fs(player().displayName));
                        }
                    }
                    catch (_e) {
                        console.log('Failure to load: %s', _e);
