...
class PlayerDaemon extends MUDObject {
    ...
    async creatorFilename(name) {
        ...
        await efuns.unguarded(async () => {
            let result = await efuns.fs.createDirectoryAsync(creatorDirectory, MUDFS.MkdirFlags.EnsurePath)
    ...
    async loadPlayerData(name, cb) {
        let cf = await this.creatorFilename(name),
            pf = await this.playerFilename(name);

        if (efuns.isFile(cf)) {
            try {
                let playerData = await efuns.readJsonFileAsync(cf);
