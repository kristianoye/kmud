...
class PlayerDaemon extends MUDObject {
    ...
    loadPlayer(name, data, callback) {
        ...
        if (efuns.isFile(cn)) {
            try {
                let playerData = efuns.readJsonFile(cn), creator = false;
                try {
                    let pfn = this.playerVirtualPath(n, true);
                    creator = efuns.findObject(pfn);
                    if (!creator) {
                        creator = efuns.loadObjectSync(pfn, playerData);
                        ChatDaemon()
                            .broadcast('announce', `${(creator().displayName)} has entered the game.`);
                    } else {
                        ChatDaemon()
    ...
    loadPlayerData(name, cb) {
        ...
        if (efuns.isFile(cf)) {
            try {
                let playerData = efuns.readJsonFile(cf);
            ...
            try {
                let playerData = efuns.readJsonFile(pf);
                return cb(playerData, false);
            }
            catch (err) {
                return cb(false, err);
    ...
    async playerExists(name, loadPlayer = false, callback = false) {
        ...
        if (loadPlayer) {
            if (creatorFile) {
                let error, exists = await efuns.readJsonFileAsync(creatorFile).catch(e => { error = e });
                if (exists)
                    return callback(exists);
                else if (error)
                    return callback(undefined, error);
            }
            let [exists, error] = await efuns.readJsonFileAsync(playerFile);
            if (exists)
                return callback(exists);
            else if (error)
                return callback(undefined, error);
            ...
            if (creatorFile) {
                let error, exists = await efuns.isFile(creatorFile).catch(e => { error = e });
                if (exists)
                    return callback(exists && creatorFile);
                else if (error)
                    return callback(undefined, error);
            }
            let [exists, error] = await efuns.isFile(playerFile);
            if (exists)
                return callback(exists && playerFile);
            else if (error)
                return callback(undefined, error);
            else
                return callback(false);
