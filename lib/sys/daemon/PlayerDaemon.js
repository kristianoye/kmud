...
class PlayerDaemon extends MUDObject {
    async createNewCharacter(data) {
        ...
        CHAT_D->broadcast('announce', `${player().displayName} has entered the world for the first time!`);
    ...
    protected async findOrLoadPlayer(name) {
        let userName = this.normalizeName(name),
            playerObject = efuns.living.findPlayer(name);

        if (playerObject)
            return playerObject;
        else
            playerObject = await this.loadPlayer(userName);

        return unwrap(playerObject);
    }

    async loadPlayer(name) {
        name = this.normalizeName(name);
        let playerFile = await this.playerFilename(name),
            creatorFile = !name.startsWith('@') && await this.creatorFilename(name);

        if (creatorFile && creatorFile.exists) {
            let creator = await efuns.objects.loadObjectAsync(creatorFile.baseName, creatorFile.fullPath);
            return creator;
        }
        else if (playerFile && playerFile.exists) {
            let player = await efuns.objects.loadObjectAsync(playerFile.baseName, playerFile.fullPath);
            return player;
        }
    }
