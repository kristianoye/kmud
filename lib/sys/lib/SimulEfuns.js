...
*/
class SimulEfuns extends EFUNProxy {
    ...
    static consolidate(c, word) {
        if (c === 1) return word;
            ...
            return SimulEfuns.cardinal(c) + ' ' +
                SimulEfuns.pluralize(
                    SimulEfuns.removeArticle(word));
    }
            ...
            default:
                if (x > 1000000000) return "over a billion";
                    ...
                    if ((x = Math.floor(x % 1000000)))
                        return `${SimulEfuns.cardinal(a)} million ${SimulEfuns.cardinal(x)}`;
                    else return `${SimulEfuns.cardinal(a)} million`;
                }
                else if ((a = Math.floor(x / 1000))) {
                    if ((x = Math.floor(x % 1000)))
                        return `${SimulEfuns.cardinal(a)} thousand ${SimulEfuns.cardinal(x)}`;
                    else return `${SimulEfuns.cardinal(a)} thousand`;
                }
                else if ((a = Math.floor(x / 100))) {
                    if ((x = Math.floor(x % 100)))
                        return `${SimulEfuns.cardinal(a)} hundred ${SimulEfuns.cardinal(x)}`;
                    else return `${SimulEfuns.cardinal(a)} hundred`;
                }
                else {
                    a = x / 10;
                    if (x = x % 10) tmp = "-" + SimulEfuns.cardinal(x);
                    else tmp = "";
                    switch (a) {
    ...
    static identify(value) {
        ...
        if (Array.isArray(target)) {
            return '[' + target.map((v, i) => {
                return SimulEfuns.identify(v);
            }) + ']';
        }
        else if (target instanceof MUDObject) {
            return `OBJ(${target.getName()} ${target.filename})`;
        }
            ...
            return '{ ' + Object.keys(target)
                .map(k => k + ':' + SimulEfuns.identify(target[k]))
                .join(', ') + ' }';
        }
            ...
            throw new Error(`Uhandled type in identify(): ${typeof target}`);
        }
    }

    static objective(target, perspective) {
        ...
        switch (x) {
            ...
            default:
                if (x > 1000000000) return "over a billion";
                    ...
                    if (x = Math.floor(x % 1000000))
                        return `${SimulEfuns.cardinal(a)} million ${SimulEfuns.ordinal(x)}`;
                    else return `${SimulEfuns.cardinal(a)} million${suffix}`;
                }
                else if (a = Math.floor(x / 1000)) {
                    if (x = Math.floor(x % 1000))
                        return `${SimulEfuns.cardinal(a)} thousand ${SimulEfuns.ordinal(x)}`;
                    else return `${SimulEfuns.cardinal(a)} thousand${suffix}`;
                }
                else if (a = Math.floor(x / 100)) {
                    if (x = Math.floor(x % 100))
                        return `${SimulEfuns.cardinal(a)} hundred ${SimulEfuns.ordinal(x)}`;
                    else return `${SimulEfuns.cardinal(a)} hundred${suffix}`;
                }
                else {
                    a = x / 10;
                    if (x = x % 10) tmp = "-" + SimulEfuns.cardinal(x);
    ...
    static possessive(target, perspective) {
        return unwrap(target, (ob) => {
            let gender = ob.gender;
            if (ob === unwrap(perspective))
                return 'your';
    ...
    static reflexive(target, perspective) {
        return unwrap(target, (ob) => {
            let gender = ob.gender;
            if (ob === unwrap(perspective))
                return 'yourself';
    ...
    static removeArticle(str) {
        return str.replace(/^(?:a|the|an|el) /i, '');
    }
}

module.exports = SimulEfuns;
