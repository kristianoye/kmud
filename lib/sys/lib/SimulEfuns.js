...
class SimulEfuns extends MUDObject {
    appendArticle(what) {
    ...
    consolidate(c, word) {
        if (c === 1) return word;
        else return this.cardinal(c) + ' ' + this.pluralize(this.removeArticle(word));
    }
            ...
            default:
                if (x > 1000000000) return "over a billion";
                    ...
                    if (x = Math.floor(x % 1000000))
                        return "{0} million {1}".fs(this.cardinal(a), this.cardinal(x));
                    else return "{0} million".fs(this.cardinal(a));
                }
                else if (a = Math.floor(x / 1000)) {
                    if (x = Math.floor(x % 1000))
                        return "{0} thousand {1}".fs(this.cardinal(a), this.cardinal(x));
                    else return "{0} thousand".fs(this.cardinal(a));
                }
                else if (a = Math.floor(x / 100)) {
                    if (x = Math.floor(x % 100))
                        return "{0} hundred {1}".fs(this.cardinal(a), this.cardinal(x));
                    else return "{0} hundred".fs(this.cardinal(a));
                }
                else {
                    a = x / 10;
    ...
    /**
        ...
        */
    identify(value) {
        var target = unwrap(value) || value;

        if (Array.isArray(target)) {
            return '[' + target.map((v, i) => { return this.identify(v); }) + ']';
        }
        ...
        switch (x) {
            ...
            default:
                if (x > 1000000000) return "over a billion";
                    ...
                    if (x = Math.floor(x % 1000000))
                        return "{0} million {1}".fs(this.cardinal(a), this.ordinal(x));
                    else return "{0} million{1}".fs(this.cardinal(a), suffix);
                }
                else if (a = Math.floor(x / 1000)) {
                    if (x = Math.floor(x % 1000))
                        return "{0} thousand {1}".fs(this.cardinal(a), this.ordinal(x));
                    else return "{0} thousand{1}".fs(this.cardinal(a), suffix);
                }
                else if (a = Math.floor(x / 100)) {
                    if (x = Math.floor(x % 100))
                        return "{0} hundred {1}".fs(this.cardinal(a), this.ordinal(x));
                    else return "{0} hundred{1}".fs(this.cardinal(a), suffix);
                }
                else {
                    a = x / 10;
                    if (x = x % 10) tmp = "-" + this.cardinal(x);
