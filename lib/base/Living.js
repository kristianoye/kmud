...
class Living extends Body {
     ...
     */
    protected async applyGetShellSettingsAsync(verb) {
        let result = {};
        ...
        let cmd = await CommandResolver().resolveAsync(verb, this.searchPath);
        if ((result = cmd && ShellFlags.Parse(cmd.getShellFlags(verb)))) 
            return result;

        let emote = EmoteDaemon().validVerbTarget(verb);
        if (emote)
    ...
    protected async executeCommand(evt) {
        ...
        if ((cmdObj = await CommandResolver().resolveAsync(evt.verb, this.searchPath))) {
            /** @type {function} */
            let method = evt.htmlEnabled && typeof cmdObj.webcmd === 'function' ?
                cmdObj.webcmd : cmdObj.cmd;

            if (efuns.isAsync(method))
                return await method(cmdObj, evt.text, evt)
                    .catch(err => {
                ...
                return method(cmdObj, evt.text, evt);
        }
        else if ((cmdResult = await EmoteDaemon().cmd(evt.verb, evt.args, evt)))
            return cmdResult;
        else
