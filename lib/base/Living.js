...
const
    ChatDaemon = efuns.loadObject(Daemon.Chat),
    CommandResolver = efuns.loadObject(Daemon.Command),
    EmoteDaemon = efuns.loadObject(Daemon.Emote);
...
class Living extends Body {
    ...
    constructor(ctx) {
        super(ctx.prop({
            ...
            searchPath: [],
            skills: {
                "unarmed combat": { level: 1, max: 5, points: 0 },
                "club weapons": { level: 1, max: 5, points: 0 }
            },
            stats: {
                strength: 1,
                constitution: 1,
         ...
         */
        function dispatchInput(evt) {
            let result = false,
                cmdWrapper = false,
                cmdResult = false;

            if ((cmdWrapper = CommandResolver().resolve(evt.verb, self.searchPath))) {
                let cmd = unwrap(cmdWrapper);
                result = evt.preferHtml && typeof cmd.webcmd === 'function' ? cmd.webcmd(evt.args, evt) : cmd.cmd(evt.args, evt);
            }
            else if ((cmdResult = EmoteDaemon().cmd(evt.verb, evt.args, evt))) result = cmdResult;
            evt.callback.call(evt.client, evt);
        }

        ctx.$storage.prependListener('kmud.command', (/** @type {MUDInputEvent} */ evt) => {
            dispatchInput(evt);
        });
    }

    dispatchInput(cmdline) {
        try {

        }
        catch (x) {
            this.writeLine(`An error occurred: ${x.message}`);
            this.writeline(x.stack || x.trace);
        }
        return true;
    }
    ...
    /**
        * Returns the primary identifier of the object.
