...
function dispatchInput(evt) {
    let result = false,
        cmdWrapper = CommandResolver().resolve(evt.verb, self.searchPath),
        cmdResult = false;

    if (cmdWrapper) {
        result = unwrap(cmdWrapper, cmd => {
            let doHtml = evt.preferHtml && typeof cmd.webcmd === 'function';
            return cmd[doHtml ? 'webcmd' : 'cmd'](evt.args, evt);
        });
    }
...
class Living extends Body {
    ...
    create($store) {
        $store.prependListener('kmud.command', (/** @type {MUDInputEvent} */ evt) => {
            dispatchInput.call(this, evt);
        });
    }
