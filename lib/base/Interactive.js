...
class Interactive extends Living {
    constructor(data) {
        ...
        efuns.livings.enableHeartbeat(true);
    }
     ...
     */
    protected executeCommand(evt) {
        try {
            let result = false, cmd = false, cmdResult = false;
            if ((cmdResult = this.executeEditor(evt))) result = cmdResult;
            else if (this.expandAliases(evt)) result = true;
            else if (this.executeHistory(evt.original)) result = true;
            else if (this.executeShellCommand(evt)) result = true;
            else if (ChatDaemon().cmd(evt.verb, evt.args, evt)) result = true;
            else if ((cmd = CommandResolver().resolve(evt.verb, this.searchPath))) {
                result = evt.htmlEnabled && typeof cmd.webcmd === 'function' ? cmd.webcmd(evt.args, evt) : cmd.cmd(evt.args, evt);
            }
            else if ((cmdResult = EmoteDaemon().cmd(evt.verb, evt.args, evt))) result = cmdResult;
            if (evt.fromHistory === false)
                this.history.push(evt.original);
            var resultType = typeof result;

            if (result === true) {
                /* command was syncronous and completed successfully */
                if (!result) this.writeLine(evt.error);
            ...
            write('You encountered an error.');
            if (efuns.wizardp(this)) {
                write(efuns.indent(err.stack));
            }
            return evt.complete(MUDEVENT_STOP, evt);
        }
    }
    ...
    protected connect(port, clientType) {
        ...
        deleteChannels.forEach(ch => {
            write(`You no longer have access to the ${ch} channel.`);
        });
    ...
    protected heartbeat(length) {
        try {
            ...
            efuns.livings.enableHeartbeat(false);
        }
    }
        ...
        if (cmd.startsWith('!')) {
            ...
            if (cmd === '!') {
                ...
                if (test.length > 0) {
                    test.forEach(index => {
                        if (!hist[index])
                            write(`History: Command index ${index} not found.`);
                        else
    ...
    movePlayer(target) {
        if (this.moveObject(target)) {
            write(this.environment.onGetDescription(this));
        }
    }
