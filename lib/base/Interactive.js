...
class Interactive extends Living {
    constructor(data) {
        ...
        register({
            interactive:
            {
                ...
                ':shellVariables': {
                    COLUMNS: () => { return efuns.clientCaps().clientWidth; },
                    LINES: () => { return efuns.clientCaps().clientHeight; },
                    TERM: () => { return efuns.clientCaps().terminalType; }
                },
            }
        });
    }
    ...
    protected executeCommand(evt) {
        ...
        if ((cmdResult = this.executeEditor(evt)))
            result = cmdResult;
        else if ((cmdResult = this.executeShellCommand(evt)))
            result = cmdResult;
        else if (ChatDaemon().cmd(evt.verb, evt))
            result = true;
        else if ((cmd = CommandResolver().resolve(evt.verb, this.searchPath))) {
            result = evt.htmlEnabled && typeof cmd.webcmd === 'function' ?
