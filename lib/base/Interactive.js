...
const
    ...
    ChatDaemon = require(Daemon.Chat),
    CommandResolver = require(Daemon.Command),
    EmoteDaemon = require(Daemon.Emote);
...
class Interactive extends Living {
    ...
    protected processInput(evt) {
        try {
            ...
            else if (this.executeHistory(evt)) result = true;
    ...
    executeHistory(cmd) {
        ...
        if (cmd === '!') {
            ...
            let test = cmd.split(',').filter(function (s, i) {
                var n = parseInt(s);
                if (typeof n === 'number' && n > -1 && hist[n]) {
                    return this.dispatchInput(hist[n], true), true;
                }
                return false;
            });
