...
class ArgSwitch extends SimpleObject {
    ...
    create(owner, opt) {
        ...
        /** @type {string} */
        this.setv = opt.setv || false;

        if (this.setv) {
            this.bitflag = false;
        }

        this.value = opt.value || undefined;
    ...
    async parse(verb, args, result) {
        ...
        if (this.parameterCount === 0 && !this.bitflag) {
            if (this.setv && typeof this.value !== 'undefined') {
                result[this.setv] = this.value;
            }
    ...
    prepareResult(result) {
        for (const p of this.parameters) {
            if (typeof p.defaultValue !== 'undefined' || p.name in result === false)
...
export default class ArgParser extends SimpleObject {
        ...
        if (this.arguments.length > 0) {
            ...
            for (let i = 0; i < this.arguments.length; i++) {
                ...
                if (isSet)
                    ...
                    let chunk = argBuffer,
                        existingValues = parm.name in result && Array.isArray(result[parm.name]) && result[parm.name] || [],
                        existingCount = existingValues.length || 0,
                        argsNeeded = Math.max((parm.minCount || 0) - existingCount, 0);

                    if (chunk.length < argsNeeded)
                        return `${verb}: Argument ${parm.name} requires at least ${argsNeeded} more values`;
                    if (parm.maxCount && chunk.length > parm.maxCount)
                        arrayBuffer = chunk.slice(0, parm.maxCount);
                    else
                        arrayBuffer = chunk.slice(0);

                    if (arrayBuffer.length > 0)
                        existingValues.push(...arrayBuffer);
                    result[parm.name] = existingValues.slice(0);
                    arrayBufferAvailableCount = Math.max(existingValues.length - (parm.minCount || 0) - existingCount, 0);
                    inc += existingValues.length;
                    argBuffer.splice(0, existingValues.length);
                }
    ...
    prepareResult(result = {}) {
        ...
        for (const arg of this.arguments) {
            //  Make sure not to pass a reference to defaultValue back
            if (Array.isArray(arg.defaultValue))
                result[arg.name] = arg.defaultValue.slice(0);
            else if (typeof arg.defaultValue === 'object')
                result[arg.name] = Object.assign({}, arg.defaultValue);
            else
