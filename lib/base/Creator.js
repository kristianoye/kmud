...
class Creator extends Player {
    ...
    constructor(ctx) {
        ...
        ctx.$storage.prependListener('kmud.command', (/** @type {MUDInputEvent} */ evt) => {
            if (executeShellCommand.call(this, evt.verb, evt.args)) {
            ...
            switch (data.eventType) {
                ...
                case 'wizShell.getDir':
                    this.eventSend({
                        ...
                        eventData: ((dir) => {
                            let result = efuns.readDirectory(dir, 1);
                            return result.map(fd => {
                                var entry = {
                                    id: dir + fd[0] + (fd[1] === -2 ? '/' : ''),
                                    text: fd[0],
                                    children: fd[1] === -2
    ...
    save(callback) {
        var PlayerDaemon = efuns.loadObject(Daemon.Player);
