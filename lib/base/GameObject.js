...
class GameObject extends MUDObject {
    ...
    get adjectives() {
        let result = get(PROP_ADJECTIVES, []);
        return result.map(s => {
    ...
    set adjectives(valueList) {
        ...
        set(PROP_ADJECTIVES, valueList);
    }

    get longDesc() {
        return get(PROP_LONGDESC, 'It is not much to look at.');
    }

    set longDesc(value) {
        set(PROP_LONGDESC, value);
    }

    get idList() {
        return get(PROP_IDLIST, []);
    }
    ...
    set idList(valueList) {
        ...
        this.setProperty(PROP_IDLIST, valueList);
    }
    ...
    get keyId() {
        return get(PROP_KEYID, 'object', PRIVATE);
    }

    protected set keyId(value) {
        if (typeof value === 'string' && value.length > 0) {
            set(PROP_KEYID, value, PRIVATE);
        }
    }
    ...
    set name(value) {
        return set(PROP_KEYID, value, PRIVATE);
    }

    get pluralIds() {
        var result = get(PROP_PLURALS, false);
        if (!result)
    ...
    set pluralIds(valueList) {
        this.setProperty(PROP_PLURALS, valueList);
    }
    ...
    get shortDesc() {
        return get(PROP_SHORTDESC, '');
    }

    set shortDesc(value) {
        if (typeof value === 'string' && value.length > 0) {
            set(PROP_SHORTDESC, value);
        }
    }
