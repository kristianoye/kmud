...
class Look extends Verb {
    ...
    doLook() {
        write(thisPlayer().environment.onGetDescription(thisPlayer));
        return true;
    }

    doLookAtObject(target) {
        write(`You look at the ${target.brief}`);
        write(target.longDescription);
        return true;
    }

    doLookAtObjectInObject(target, container) {
        write(`You look at the ${target.shortDescription} in ${container.shortDescription}`);
        return true;
    }
