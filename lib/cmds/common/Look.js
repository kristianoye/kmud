...
class Look extends Verb {
    ...
    doLook() {
        writeLine(thisPlayer().environment.onGetDescription(thisPlayer));
        return true;
    }

    doLookAtObject(target) {
        writeLine(`You look at the ${target.brief}`);
        writeLine(target.longDescription);
        return true;
    }

    doLookAtObjectInObject(target, container) {
        writeLine(`You look at the ${target.shortDescription} in ${container.shortDescription}`);
        return true;
    }
