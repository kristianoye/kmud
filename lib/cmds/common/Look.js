...
class Look extends Verb {
    ...
    doLookAtObject(target) {
        thisPlayer.writeLine(`You look at the ${target.shortDescription}`);
        thisPlayer.writeLine(target.longDescription);
        return true;
    }

    doLookAtObjectInObject(target, container) {
        thisPlayer.writeLine(`You look at the ${target.shortDescription} in ${container.shortDescription}`);
        return true;
    }
