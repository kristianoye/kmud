...
class Look extends Verb {
    ...
    doLook() {
        efuns.write(thisPlayer().environment.onGetDescription(thisPlayer));
        return true;
    }

    doLookAtObject(target) {
        efuns.write(`You look at the ${target.shortDescription}`);
        efuns.write(target.longDescription);
        return true;
    }

    doLookAtObjectInObject(target, container) {
        efuns.write(`You look at the ${target.shortDescription} in ${container.shortDescription}`);
        return true;
    }
