...
class Alias extends Command {
    cmd(args, cmdline) {
        ...
        if (args.length === 0) {
            if (list.length === 0) {
                write('You have not created any aliases');
            }
            else {
                for (var k in aliases) {
                    write(`alias ${k}='${aliases[k]}'`);
                }
            }
        }
        else
        {
            ...
            if (m && m.length > 1) {
                write(`Alias ${(typeof aliases[m[1]] !== 'undefined' ? 'altered' : 'created')}: ${m[1]}=${m[2]}`);
                aliases[m[1]] = m[2];
            }
                    ...
                    case 'r':
                        ...
                        if (args.length === 2) {
                            if (typeof aliases[args[1]] !== 'undefined') {
                                ...
                                write(`alias: ${args[1]}: removed`);
                            }
                            else {
                                write(`alias: ${args[1]}: not found`);
                            }
                        }
                        else
                ...
                if (foo) {
                    write(`alias ${args[0]}='${foo}`);
                } else {
                    write(`alias ${args[0]}: not found`);
                }
            }
        }
        return true;
