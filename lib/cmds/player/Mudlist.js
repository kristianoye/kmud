...
class Mudlist extends Command {
    cmd(args, cmdline) {
        if (args.length === 0) {
            ...
            mudList.forEach(mud => {
                write(efuns.sprintf('%-20s %-10s %-10s', mud.name, mud.mudlib, mud.driver));
            });
            if (mudList.length === 0) {
                write('No MUDs found at this time.');
            }
            return true;
        }
        else {
            ...
            for (var i = 0; i < args.length; i++) {
                ...
                if (!norm[name])
                    write(`There is no MUD named ${name} that we are aware of.`);
                else {
                    write('MUD Name: ' + JSON.stringify(norm[name]));
                }
            }
        }
    }
